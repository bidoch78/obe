...
var ytable_template_bootstrap4 = function() {
	
	this.buildTable = function(yt) {
		
		var fields = yt.getFields();
		
		var tableClass = "table table-sm table-striped";
		if (yt._options.selectable) tableClass += ' table-hover';
		
		yt.getContainer().find("table").addClass(tableClass);
	...
	this.buildPagination = function(yt, pagination) {
		...
		var html = "";
		html += '<tr><td colspan="' + nbFieldsDisp + '"><div>';
	...
	this.addRecord = function(yt, data, $th) {
		...
		$th.find(".ytselect").on("click", $.proxy(function(e) {
			...
			this.onSelectTR($this.is(":checked"), $tr);
		}, yt));
	...
	this.onSelect = function(state, $tr) {
		if (state) { $tr.addClass("table-warning"); } else { $tr.removeClass("table-warning"); }
